...
- [x] Terminal Wrench arm DROPPED (Yoonho, 2026-07-31): no hardened verifiers exist at the pins, so the original-vs-hardened headline was unrunnable as specified; setup evidence (pins, W=30, exclusions, passing gate) kept in the 002 buildlog and external-benches/splits/ for any revival; twbuild-* docker images pruned.
- [x] TB2 easy split ruled (Yoonho, 2026-07-31): E = 30, the seed-0 sample over the 89 paired tasks, promoted to `E` in tb2_easy_split.json (tag rule's E=4 kept as superseded history); sweep is 3 artifacts x 2 benches x 30 x k=3 = 540 trials once h*_10 exists.
