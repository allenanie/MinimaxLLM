...
- [x] `scripts/bench_modal.py` done (1e73a6d): $0.143/trial pooled, --concurrency 16 for the game, budget restated at k=2 in the 002 buildlog (~$290-420 grand total); table reprints from stored job dirs with zero respend.
...
- [ ] Terminal Wrench arm: setup done 2026-07-31 (W=30 fixed, OT-lineage exclusions applied, schema drop-in, gate passed for seed and 0b93de56), but the rescore-vs-two-arm question resolved to NEITHER: no hardened verifiers exist at the pinned commits (terminal-wrench ships only original_task dirs; harden-v0 is loop code with no task data). **Yoonho decision needed before the sweep**: (a) run original-verifier pass rates only (weak headline), (b) generate hardened verifiers with harden-v0's adversarial loop (two-arm, doubled cost, a separate project), or (c) drop the arm. Evidence in the 002 buildlog and external-benches/splits/terminal_wrench_split.json.
- [ ] TB2 easy split: the primary rule (TB2 difficulty tags) yields E=4 only, and Fn's own tags disagree on 3 of the 4; the seed-0 sample of 30 is pre-recorded in tb2_easy_split.json as `alternate_seed0_30_NOT_THE_SPLIT`. **Yoonho call before the sweep**: keep the rule's honest E=4 (72 sweep trials) or switch to the 30-task sample (540 trials); gate for seed and 0b93de56 already passed on fix-git both benches.
...
- [ ] **002 steps 3-5 (round loop, loop controls, the run): blocked on Yoonho's judge-spec edit.** Step 2b controls FAILED 5/6 (2026-07-31): all four must-gate moves gated g=0 and the sane harness change passed g=1, but the sane instruction-tightening variant gated g=0 deterministically (3/3) because the draft spec's answer-source clause reads on the corpus-wide /solution notice line every task carries; the verbatim spec kills the adversary's variant channel. Evidence: results/step2b-controls/verdicts.json and the 002 buildlog. Needed from Yoonho: the pre-freeze spec sentence (scope the clause to what the diff introduces, or exempt the corpus notice line) and a ruling on whether variants must keep the notice line (else deleting it is the adversary's rational move and disclosed bait becomes hidden bait; the edit-surface check can mechanically enforce keeping it if ruled). After his edit: re-run the six 2b controls, freeze, then steps 3-5 proceed.
