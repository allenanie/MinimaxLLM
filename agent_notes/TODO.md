...
- [ ] Builder-adversary game: draft dispatch at `agent_notes/dispatch/002-builder-adversary-game/dispatch.md`. Decisions settled 2026-07-30 (task-editing adversary, incumbent source visible, strict gate, counterfactual arm as core metric); in-grid regret payoffs and the 3x3 grid folded in 2026-07-31 per `design.md` (source of truth), superseding the 2026-07-30 reward-payoff ruling; awaiting Yoonho's judge-spec edit and `## Status: approved`. Judge is per-move (per harness + per new task), 10 rounds, steps reordered (judge kill-test at 2b before the loop), TB2 + TB2-Fn easy-split transfer added as step 6. Remaining 2026-07-31 refinements (rubric judge, drop counterfactual metric, real filesystem sandbox per role, single-writer crash-safe resume, diff-based evidence) not yet folded into the dispatch.
- [ ] `scripts/bench_modal.py`: benchmark harbor-on-modal speed, throughput, price, and concurrency scaling; plan is the "Modal throughput bench" section of the 002 dispatch; spends modal + solver money, so launch is a separate go from Yoonho.
