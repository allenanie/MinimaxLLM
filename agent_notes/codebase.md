...
- `oracle.py`: c* over plain trajectory dicts: path-component contact (touched/acted/exploited) plus gold provenance with distinctiveness, shipped-text, and temporal-exposure screens.
...
- `oracle.py::score`: produces the label every downstream number is about; rescored offline by `report_001.py`; pinned by most of `tests.py`.
